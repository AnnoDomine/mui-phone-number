...
describe('useMuiPhoneNumber', () => {
  ...
  it('should handle preferredCountries', () => {
    ...
    expect(
      result.current.computedPreferredCountries.some((c) => c.iso2 === 'gb'),
    ).toBe(true);
    expect(
      result.current.computedPreferredCountries.some((c) => c.iso2 === 'de'),
    ).toBe(true);
  });
  ...
  it('should filter by regions', () => {
    ...
    expect(
      result.current.computedOnlyCountries.every((c) =>
        c.regions.includes('north-america'),
      ),
    ).toBe(true);
    expect(
      result.current.computedOnlyCountries.some((c) => c.iso2 === 'us'),
    ).toBe(true);
    expect(
      result.current.computedOnlyCountries.some((c) => c.iso2 === 'gb'),
    ).toBe(false);
  });
});
