...
describe('MuiPhoneNumber UI tests', () => {
  // biome-ignore lint/suspicious/noExplicitAny: vitest props
  ...
  it('should render and allow typing', async () => {
    ...
    await waitFor(() => expect(input).toHaveValue('+1 (702) 123-4567'), {
      timeout: 5000,
    });
  });
  ...
  it('should open the country dropdown and select a country', async () => {
    ...
    const germanyItem = await screen.findByText('Germany', {
  ...
  it('should handle preferred countries correctly', async () => {
    const user = userEvent.setup();
    render(
      <MuiPhoneNumber defaultCountry="us" preferredCountries={['gb', 'de']} />,
    );
